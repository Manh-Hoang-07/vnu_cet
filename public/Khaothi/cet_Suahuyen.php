...
//---------------------------------------Mô tả-----------------------------------------------
// Module: Suahuyen.php
// Chức năng: Sửa huyênk /quận/Thị xã
// Phiên bản: 1.1
...
<title>Cập nhật thông tin quận/huyện</title>
<style>
...
function check1() {//alert('c1');
	if((document.cet_Suahuyen.TenhuyenS.value=="0")||(document.cet_Suahuyen.TenhuyenS.value==""))	{
		alert('Tên quận / huyện chưa hợp lệ!'); 
		document.cet_Suahuyen.TenhuyenS.focus(); 
		return false;
		}
		if((document.cet_Suahuyen.PhamviS.value=="0")||(document.cet_Suahuyen.PhamviS.value==""))	{
		alert('Loại đơn vị hành chính chưa hợp lệ!'); 
		//document.cet_Suahuyen.TenhuyenS.focus(); 
		return false;
		}
	return true;
...
function check(){
	...
	{
		//alert('c OK');
		document.cet_Suahuyen.Sendcheck.value="Ghi nhậnOK"; 
		document.cet_Suahuyen.submit();
		}
} 

function confirm2(){
	if(confirm('Bạn muốn loại bỏ trường?')){
		document.cet_Suahuyen.Sendcheck.value="Ghi hủy"; 
		document.cet_Suahuyen.submit();
	}
}
function confirm3(){
	if(confirm('Bạn muốn khôi phục trường?')){
		document.cet_Suahuyen.Sendcheck.value="Ghi khôi phục"; 
		document.cet_Suahuyen.submit();
	}
}
// -->
...
//-------------------------------------------------------------------------------------------------------------
	...
	$TenhuyenS = ($_POST['TenhuyenS']);
	$PhamviS = ($_POST['PhamviS']);
...
//------------------Cập nhật Môn thi -----------------------------------------------
	...
	if($codeOK =="Ghi nhậnOK"){ //me('Ghi OK:'.$Matruonggoc);
		...
		$sql = 'UPDATE quanhuyen SET  	Tenquanhuyen = "'.$TenhuyenS.'", Phamvi = "'.$PhamviS.'",Ghichu = "'.$GhichuS.'" WHERE (Mahuyen ="'.$MahuyenS.'")';
...
//------------------Tiêu đề trang------------------------------	----------------------------------------------
	...
	print '<tr><td width="40%" rowspan="2" style="font-size: 24pt; color: #0000FF" >&nbsp; Cập nhật thông tin quận huyên</td><td width="30%">&nbsp;&nbsp;&nbsp;</td><td width="30%">&nbsp;&nbsp;&nbsp;</td></tr>';
	...
	print '</table>';
	print '<form action="cet_Suahuyen.php" method="post" name ="cet_Suahuyen" enctype ="multipart/form-data">' ;
	print '<hr>';
	print '<table><tr>';
...
{ // Chưa nhập dữ liệu sửa, --> đọc từ CSDL
	$Mahuyen =$Mahuyen0;
	$sql ='SELECT Mahuyen, Tenquanhuyen, Phamvi,Trangthai,Ghichu	FROM quanhuyen 
			WHERE (Matinh ="'.$Matinh0.'" AND Tenquanhuyen ="'.$Mahoso.'")';
	me($sql);
	$result = mysql_query($sql, $link);
	if (!$result) {	thongbaoloi('Lỗi đọc dữ liệu trường :' . $sql);	 exit;	}
	...
	$row = mysql_fetch_row($result);			
	$MahuyenS= $row[0]; $TenhuyenS= $row[1]; $KhuvucS= $row[2];$TrangthaiS= $row[3];
	$GhichuS= $row[4];
...
'<br><div class="rowdiv" style="clear:both;width:80%">
	...
	print '<fieldset> <legend><b>Cập nhật thông tin quận / huyện </b></legend>';
	
	print '<table  width="100%" border="0" style="font-family: Times New Roman; font-size: 16pt">';
	
	print '<tr height ="'.$Height.'"><td width="20%"> Tên quận / huyện :</td><td> 
			<input type = "textbox" name = "TenhuyenS" value ="'.$TenhuyenS.'" style="height:'.$Height1.';width:60%;font-size: 12pt"></td>';
	print '<input type ="hidden" name ="MahuyenS" value ="'.$MahuyenS.'"></td><td></td></tr>';
	print '<tr height ="'.$Height.'">';
	print '<td>Đơn vị hành chính: </td><td><input type ="radio" name = "PhamviS"  Id = "PhamviS"  value ="1"> Quận
				&emsp;<input type ="radio" name = "PhamviS"  Id = "PhamviS"  value ="2"> Thị xã
				&emsp;<input type ="radio" name = "PhamviS"  Id = "PhamviS"  value ="3"> Huyện </td>';
	print '<td>Trạng thái: '.($TrangthaiS == "0"? "bình thường":"đã xóa");
	print '</td></tr>';
	print '<tr height ="'.$Height.'"><td >Thông tin thêm:   </td><td colspan="2"><input type = "textbox" name = "GhichuS"  value = "'.$GhichuS.'" style="height:'.$Height1.';width:95%;font-size: 12pt"></tr>';
	...
	if($TrangthaiS =="0")
		...
		print '&nbsp;&nbsp;<input name="Delete" type="button" onclick ="return confirm2();" value = "Hủy tên huyên" style="height:27px;font-size:12pt;font-weight:bold;width:120pt">';
	else 
	...
	print '<script>cet_Suahuyen.elements["Sendback"].focus();</script>';
print '</table>';
}//Sửa
	print '</form>';
