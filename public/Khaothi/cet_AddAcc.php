...
//-----------------------/Các hàm-------------------------------------------
	...
	$Ausername   = $_SESSION['cetAusbaomat'];
	$Apassword	= $_SESSION['cetpAusbaomat'];
	if(empty($Ausername)) {thongbaoloi1("Bạn chưa đăng nhập"); exit;}
	
	if (!$link = cet_Aconnect($Ausername, $Apassword)) {thongbaoloi('Đăng nhập không hợp lệ !'); exit;}
	mysql_query("SET NAMES utf8");	
	$auth = Get_name($link,"cetusers","Mucquyen","Tendangnhap",$Ausername);
...
if($todo =="Ghi nhận"){
	...
	$sql    = 'Select tendangnhap from cet_student_acc where (tendangnhap ="'.$username.'")';
	...
	if(mysql_num_rows($result)>0) { thongbaoloi("Trùng tên đăng nhập"); }
		...
		if(mysql_num_rows($result)>0) { thongbaoloi("Trùng email"); }
			...
			$sql    = 'Insert into cet_student_acc (Hoten, Sodienthoai, Email,tendangnhap,Matkhau, Trangthai) 
			value ("'.$hoten.'","'.$sodienthoai.'","'.$email.'","'.$username.'","'.$password.'",0)';
			$result = mysql_query($sql, $link);
